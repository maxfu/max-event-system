...
class Upcoming_Events extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description'	=>	__( 'A widget to display a list of upcoming events', 'maxfu-events-system' )
		);

		parent::__construct(
			'mes_upcoming_events',			//base id
			__( 'Upcoming Events', 'maxfu-events-system' ),	//title
			$widget_ops
		);
	}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title', 'maxfu-events-system' ); ?></label>
			<input type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" class="widefat" value="<?php echo esc_attr( $instance['title'] ); ?>">
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'number_events' ); ?>"><?php _e( 'Number of events to show', 'maxfu-events-system' ); ?></label>
		...
		<ul class="mes_event_entries">
			...
			?>
				<li class="mes_event_entry">
					<h4><a href="<?php the_permalink(); ?>" class="mes_event_title"><?php the_title(); ?></a> <span class="event_venue">at <?php echo $event_venue; ?></span></h4>
					<?php the_excerpt(); ?>
					<time class="mes_event_date"><?php echo date( 'F d, Y', $event_start_date ); ?> &ndash; <?php echo date( 'F d, Y', $event_end_date ); ?></time>
				</li>
			<?php endwhile; ?>
		</ul>
...
function mes_register_widget() {
	register_widget( 'Upcoming_Events' );
}
add_action( 'widgets_init', 'mes_register_widget' );
